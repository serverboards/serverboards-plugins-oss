...
import time
import serverboards
from serverboards import cache_ttl, print
...
class Connection:
    ...
    def details(self, node, extra_info=False):
        ...
        if extra_info:
            ...
            try:
                # from GiB to MiB
                extra["disk_total"] = extra["image"]["block_device_mapping"][0]["ebs"]["volume_size"] * 1024
            except:
                pass
        ...
        if extra_info:
            ...
            if used_cpu_time:
                ...
                if node.uuid in self.prev_used_cpu_time_v:
                    extra["CPU_rt"] = (used_cpu_time-self.prev_used_cpu_time_v[node.uuid]
                                       )/(now-self.prev_used_cpu_time_t[node.uuid])
