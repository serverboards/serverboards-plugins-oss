...
class Cache:
    ...
    def get_lock(self, hv):
        lock = self._locks.get(hv)
        if not lock:
            lock = curio.Lock()
            self._locks[hv] = lock
        return lock
                ...
                async with lock:
                    ...
                    if not v:
                        ...
                        if not _ttl:
                            _ttl = ttl
                        if callable(_ttl):
                            _ttl = _ttl(*args, **kwargs, key=hv, value=v)
                        self.set(hv, v, _ttl)
                return v
...
def test(cache):
    ...
    cache.set("test", "ttl", ttl=lambda a, b: 1000)
    for x in cache.keys_ttl():
        if x["key"] == "test":
            assert x["ttl"] >= 990
...
def main(argv):
    ...
    print(
        "Use as library or set command: "
        "keys | set <k> <v> | get <k> | del <k> | clean | vacuum | test"
    )
