...
function View(a){return React.createElement("div",{className:"ui google drive"},a.days.map(function(b){return React.createElement(DriveDay,_extends({key:b.date},b))}))}var drive=void 0;
var DriveWidget=React.createClass({displayName:"DriveWidget",getInitialState:function getInitialState(){return{lines:[],loading:!0,link:!1}},update:function update(){var _this=this,a=this.props.config.folder_filter.split(",").map(function(b){return b.trim()}).filter(function(b){return""!=b});drive.call("get_changes",[this.props.config.service,a]).then(function(b){_this.setState({lines:b,loading:!1})}).catch(function(b){"invalid_grant"==b?drive.call("authorize_url",[_this.props.config.service]).then(function(c){_this.props.setError("Google Drive grant has expired and was not automatically refreshed. Click [here]("+c+") to renew.")}):(_this.props.setError(b),Flash.error(b))})},componentDidMount:function componentDidMount(){this.props.setClass("ui white card with scroll"),plugin.start("serverboards.google.drive/daemon").then(function(a){drive=a}).then(this.update)},render:function render(){return this.state.loading?React.createElement(Components.Loading,null,"Google Drive Changes"):React.createElement(View,{days:this.state.lines})}});
