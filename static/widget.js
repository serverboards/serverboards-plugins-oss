...
var Model=React.createClass({displayName:"Model",getInitialState:function getInitialState(){var a=store.getState();return console.log(a),{updating:!1,expirations:void 0,project:a.project.project,service_by_uuid:void 0}},componentDidMount:function componentDidMount(){var _this=this;plugin.start_call_stop("serverboards.expiration/command","list_expirations",[]).then(function(a){if(a.updating){var b=event.on("action.updated",_this.progressUpdate),c=event.on("action.stopped",_this.progressStop);_this.setState({updating:{id:a.updating,update_id:b,stop_id:c}})}else _this.setState({expirations:a})}).catch(function(a){Flash.error(a)}),cache.services().then(function(a){var b={};a.map(function(c){return b[c.uuid]=c}),_this.setState({service_by_uuid:b})})},progressUpdate:function progressUpdate(a){var b=this.state.updating.id;b==a.uuid&&this.setState({updating:merge(this.state.updating,{progress:a.progress,label:a.label})})},progressStop:function progressStop(a){var b=this.state.updating.id;b==a.uuid&&(this.componentWillUnmount(),this.setState({updating:!1}),this.componentDidMount())},componentWillUnmount:function componentWillUnmount(){this.state.updating&&(event.off("action.updated",this.state.update_id),event.off("action.stopped",this.state.stop_id))},handleShowService:function handleShowService(a){store.goto("/project/"+this.state.project.shortname+"/services/"+a)},handleReload:function handleReload(){var _this2=this;Flash.info("This will take long time"),rpc.call("action.trigger_wait",["serverboards.expiration/update",{}]).then(function(){Flash.success("Expirations list updated"),_this2.componentDidMount()}).catch(function(a){return Flash.error(a)})},getServiceByUUID:function getServiceByUUID(a){return this.state.service_by_uuid[a]},render:function render(){return this.state.updating?React.createElement(Loading,null,"Updating expirations. This will take long time.",React.createElement("div",{className:"ui text grey"},this.state.updating.label," (",(this.state.updating.progress||0).toFixed(2),"%)")):this.state.expirations&&this.state.service_by_uuid?React.createElement("div",{className:"ui expirations "+this.props.layout},React.createElement(Timeline,{expirations:this.state.expirations,onShowService:this.handleShowService,getServiceByUUID:this.getServiceByUUID}),React.createElement("button",{className:"ui button yellow with icon",onClick:this.handleReload},React.createElement("i",{className:"ui refresh icon"}))):React.createElement(Loading,null,"Expirations")}});
