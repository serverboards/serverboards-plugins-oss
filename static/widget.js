...
var Model=React.createClass({displayName:"Model",getInitialState:function getInitialState(){return{lines:[],loading:!0}},update:function update(){var _this=this,a=this.props.config.folder_filter.split(",").map(function(b){return b.trim()}).filter(function(b){return""!=b});drive.call("get_changes",[this.props.config.service.uuid,a]).then(function(b){_this.setState({lines:b,loading:!1})})},componentDidMount:function componentDidMount(){plugin.start("serverboards.google.drive/daemon").then(function(a){drive=a}).then(this.update)},render:function render(){return this.state.loading?React.createElement(Components.Loading,null,"Google Drive Changes"):React.createElement(View,{days:this.state.lines})}});
